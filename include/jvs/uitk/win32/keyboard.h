...
static std::string GetKeyText(KeyState key)
{
  const int MaxKeyTextSize = 64;
  std::wstring ret(MaxKeyTextSize, 0);
  auto retsize = ::GetKeyNameTextW(static_cast<long>(key.LParam), &ret[0], 
    static_cast<int>(ret.size()));
  ret.resize(retsize);
  return jvs::Narrow(ret);
}
...
{
  return static_cast<wchar_t>(::towupper(ch) - 0x40);
}
  ...
  const LParam& lParam, long when)
{
  if (when == 0)
  {
    when = static_cast<long>(std::chrono::system_clock::now()
      .time_since_epoch().count());
  }

  UInt keyChar = ::MapVirtualKeyW(static_cast<UInt>(wParam), MAPVK_VK_TO_CHAR);
  KeyState keyState;
  ...
  const LPARAM& lParam, long when)
{
  ...
  KeyEventArguments e
    ...
    static_cast<UINT>(wParam), // key character
    Keyboard::GetKeyText(keyState), // key text
    Keyboard::Shift(), // shift key
    Keyboard::Ctrl(), // control key
    (lParam >> 29) & 1, // alt key
